"""County social-mobility clustering: 16 SVI vulnerability measures plus upward
mobility, urban-rural as context.
...
from src.evaluate import k_sweep, picks
from src.model import cluster, standardize
from src.utils import (COLS, FEATS, FIGURES, INK, K, LABELS, MUT, NAMES, N_ESTIMATORS,
                       OUTPUTS, PAL, POP_FLOOR, SEED, SVI_ONLY, TEST_SIZE, CV_FOLDS)
...
labels, cent, _ = cluster(df, COLS, K)
df["cluster"] = labels
...
ari_hier = adjusted_rand_score(df["cluster"], hier)
print(f"\n=== hierarchical check ===")
...
Xtr, Xte, ytr, yte = train_test_split(Xs, y, test_size=TEST_SIZE, random_state=SEED)

lin = LinearRegression().fit(Xtr, ytr)
rf = RandomForestRegressor(n_estimators=N_ESTIMATORS, random_state=SEED, n_jobs=-1).fit(Xtr, ytr)
...
df["pred"] = cross_val_predict(RandomForestRegressor(n_estimators=N_ESTIMATORS, random_state=SEED,
                                                     n_jobs=-1), Xs, y, cv=5)
df["residual"] = df["MOBILITY"] - df["pred"]
big = df[df.E_TOTPOP >= POP_FLOOR].copy()   # small-county mobility estimates are noisy
...
lab_a, cent_a, Xa = cluster(df, KEPT, K)
ari_abl = adjusted_rand_score(df["cluster"], lab_a)
sil_full, sil_abl = silhouette_score(X, df["cluster"]), silhouette_score(Xa, lab_a)
...
out.to_csv(OUTPUTS / "combined_clusters.csv", index=False)
print("saved outputs/combined_clusters.csv (now with predictions + residuals)")
